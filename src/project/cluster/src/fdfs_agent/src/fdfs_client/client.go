...
func getTrackerConf(trackerlist []string) (*Tracker, error) {
	var (
...
func NewFdfsClient(trackerlist []string, lg *log.Logger, minConns int, maxConns int) (*FdfsClient, error) {
	logger = lg
	tracker, err := getTrackerConf(trackerlist)
	if err != nil {
		return nil, err
	}

	trackerPool, err := NewConnectionPool(tracker.HostList, tracker.Port, minConns, maxConns)
	if err != nil {
		return nil, err
	}
